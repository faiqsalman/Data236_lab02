...
export default function Signup({ onClose, onSwitchToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSwitchToLogin() // after signup, prompt user to log in
    } catch (err) {
  ...
  return (
    <div className="bg-white w-full max-w-sm rounded-xl shadow-2xl px-8 py-10 flex flex-col items-center">
      ...
      <p className="text-sm text-gray-500 mb-4">Connect with great local businesses</p>
      ...
      <p className="text-sm text-gray-600">
        Already on Yelp?{' '}
        <button onClick={onSwitchToLogin} className="text-[#d32323] font-semibold hover:underline">
          Log in
        </button>
      </p>
    </div>
